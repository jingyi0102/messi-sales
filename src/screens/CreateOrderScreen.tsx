 ...
 * New order form — date pickers, required field indicators, inline validation.
 */
...
import { PrimaryButton, AppHeader, DatePickerField } from '../components';
import { useAppStore } from '../store';
import { useAuth } from '../store';
import { orderService } from '../services/orderService';
import { formatBMNumber } from '../utils/format';
import { getMonthKey, todayISO } from '../utils/date';
import { computeBalance, computeProfit, roundCurrency } from '../utils/calculation';
import type { CreateOrderScreenProps } from '../types/navigation';
...
export function CreateOrderScreen({ navigation }: CreateOrderScreenProps) {
  ...
  // Validation error flags — only shown after first submit attempt
  const [submitted, setSubmitted] = useState(false);

  // Pre-fill next BM number
  useEffect(() => {
    orderService.getLastBMNumber().then((n) => setOrderNo(formatBMNumber(n + 1)));
  }, []);

  // Pre-select first salesperson
  useEffect(() => {
    if (currentUser?.role === 'salesman') {
      setSalesperson(currentUser.displayName);
      return;
    }
    if (salespeople.length > 0 && !salesperson) {
      setSalesperson(salespeople[0].name);
    }
  }, [currentUser, salespeople, salesperson]);

  // Live calculations
  const balanceCalc = roundCurrency(
    computeBalance(Number(amount) || 0, Number(deposit) || 0, Number(additionalDeposit) || 0),
  );
  const profitCalc = roundCurrency(computeProfit(Number(amount) || 0, Number(costing) || 0));
  ...
  const errors = {
    ...
    costing: costing !== '' && Number(costing) < 0,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      dispatch({ type: 'SET_MONTH', payload: getMonthKey(newOrder.date) });
      await orderService.incrementBMNumber();

      const nextN = await orderService.getLastBMNumber();
      setOrderNo(formatBMNumber(nextN + 1));
      setManualSO('');
      setAmount('');
      setDeposit('');
      ...
      Alert.alert('Saved', `${newOrder.orderNo} has been recorded.`);
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <AppHeader title="New Order" subtitle="MESSI Sales" />
      <ScrollView
        style={styles.scroll}
      ...
      >
        ...
        <View style={styles.row}>
          <View style={styles.halfField}>
            ...
            <StyledInput
              ...
              placeholder="0.00"
              keyboardType="decimal-pad"
              error={submitted && errors.deposit}
            />
          </View>
          <View style={styles.halfField}>
        ...
        {currentUser?.role === 'admin' && (
          <>
            ...
            <StyledInput
              ...
              placeholder="0.00"
              keyboardType="decimal-pad"
              error={submitted && errors.costing}
            />
          </>
        )}
        ...
        <View style={styles.calcCard}>
          <View style={styles.calcHalf}>
            ...
            <Text style={[styles.calcValue, { color: balanceCalc > 0 ? colors.warning : colors.success }]}>
              {balanceCalc.toLocaleString('en-US', { maximumFractionDigits: 2 })}
            </Text>
          </View>
          {currentUser?.role === 'admin' && (
            <>
              ...
              <View style={styles.calcHalf}>
                ...
                <Text style={[styles.calcValue, { color: profitCalc >= 0 ? colors.success : colors.danger }]}>
                  {profitCalc.toLocaleString('en-US', { maximumFractionDigits: 2 })}
                </Text>
              </View>
            </>
          )}
        ...
        {currentUser?.role === 'salesman' ? (
          <View style={styles.lockedField}>
            <Text style={styles.lockedFieldText}>{currentUser.displayName}</Text>
          </View>
        ) : (
          <PickerRow
            options={spNames}
...
const styles = StyleSheet.create({
  ...
  lockedFieldText: { fontSize: fontSize.md, fontWeight: fontWeight.semibold, color: colors.textPrimary },
